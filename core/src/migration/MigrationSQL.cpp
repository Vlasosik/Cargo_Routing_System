...
MigrationSQL::MigrationSQL(const std::string &user, const std::string &password, const std::string &dbname,
                           const int port, const std::string &host) {
    sqlpp::postgresql::connection_config config;
    config.user = user;
...
void MigrationSQL::executeMigrations(const std::string &migrationFolder) {
    try {
        for (const auto &entry: std::filesystem::directory_iterator(migrationFolder)) {
            if (entry.is_regular_file() && entry.path().extension() == ".sql") {
                if (std::ifstream migration_file(entry.path()); migration_file) {
                    std::string migration_sql((std::istreambuf_iterator(migration_file)),
